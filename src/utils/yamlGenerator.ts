import type { DeploymentConfig, DaemonSetConfig, KubernetesResource, Namespace, ConfigMap, Secret, ServiceAccount, ProjectSettings, JobConfig, CronJobConfig, Container, EnvVar, DockerHubSecret } from '../types';
...
data:
  ...
  // Add header comment
  if (deployments.length > 0 || daemonSets.length > 0 || customNamespaces.length > 0 || configMaps.length > 0 || secrets.length > 0 || dockerHubSecrets.length > 0 || jobs.length > 0 || cronjobs.length > 0 || serviceAccounts.length > 0) {
